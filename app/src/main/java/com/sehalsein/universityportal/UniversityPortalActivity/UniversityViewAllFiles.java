...
public class UniversityViewAllFiles extends AppCompatActivity {
    ...
    private void updateDatabaseWithImage(final String url) {
        ...
        mCollegeFileRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot snapshot) {
                ImagesDetail imagesDetailList = new ImagesDetail(url);
                mCollegeFileRef.child(snapshot.getChildrenCount() + "").setValue(imagesDetailList).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        makeToast("Database Updated Successful");
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        makeToast("Database Update Failed");
                    }
                });
            }
            @Override
